...
import { PlatformActivityList, platformTones, PlatformLineChart, PlatformMetricCard, PlatformSidebar } from "./platform-ui";
    ...
    <div className="grid min-h-52.5 grid-cols-1 items-center gap-3.5 sm:grid-cols-[minmax(120px,1fr)_1fr]">
      <div className="relative grid size-35.5 place-items-center justify-self-center" aria-label={`${items[0]?.value ?? 0}% organic traffic`}>
        <svg className="absolute inset-0 size-full -rotate-90" viewBox="0 0 42 42" aria-hidden="true">
          ...
          {segments.map(({ item, offset }) => <circle className="fill-none stroke-5" style={{ stroke: platformTones[item.tone] }} cx="21" cy="21" r="15.9" pathLength="100" strokeDasharray={`${item.value} ${100 - item.value}`} strokeDashoffset={-offset} key={item.label} />)}
        </svg>
        <strong className="font-display text-xl">{items[0]?.value ?? 0}%</strong>
      </div>
      <ul className="grid list-none gap-3 p-0">{items.map((item) => <li className="grid grid-cols-[9px_1fr_auto] items-center gap-2 text-xs" key={item.label}><span className="size-2 rounded-full" style={{ background: platformTones[item.tone] }} /><small className="text-[#a3abb7]">{item.label}</small><strong>{item.value}%</strong></li>)}</ul>
    </div>
  );
}
  ...
  const dashboard = (
    <div className="grid min-h-197.5 grid-cols-1 overflow-hidden rounded-3xl border border-line-strong bg-surface sm:grid-cols-[150px_minmax(0,1fr)] xl:grid-cols-[190px_minmax(0,1fr)]" aria-label={content.ariaLabel}>
      ...
      <div className="min-w-0 p-4 sm:p-6 xl:p-7.5">
        ...
        <section className="mt-4.5 rounded-[14px] border border-[#293340] bg-surface p-5"><strong className="text-xs">{content.activityTitle}</strong><PlatformActivityList items={content.activity} /></section>
      </div>
    </div>
  );
...
export function PlatformAdminSection({ content }: { content: AdminContent }) {
  return (
    <section className="section-space scroll-reveal scroll-mt-24 border-t border-line bg-[#0d1117]" data-scroll-reveal id="admin-features">
      <div className="container"><SectionHeading {...content.heading} /><ul aria-label={content.tabsAriaLabel} className="mt-11.5 mb-8.5 flex list-none flex-nowrap gap-3 overflow-x-auto p-0">{content.tabs.map((tab, index) => <li className={`inline-flex min-h-11.5 shrink-0 items-center rounded-xl border px-6 text-xs font-semibold ${index === 0 ? "border-gold bg-gold text-[#191006]" : "border-[#2a3442] bg-[#171d27] text-[#a3abb7]"}`} key={tab}>{tab}</li>)}</ul><AdminDashboard content={content.dashboard} /></div>
    </section>
  );
}
