    ...
    <section className="section-space scroll-reveal relative" data-scroll-reveal>
      <div className="container">
        <div className="mb-12 max-w-262.5 md:mb-16">
          <p className="mb-8 inline-flex min-h-10 items-center rounded-full border border-line-strong bg-surface-strong px-6 text-sm font-extrabold text-gold-bright uppercase">{content.heading.label}</p>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          {content.items.map((reason) => (
            <article className="scroll-reveal flex min-h-65 flex-col rounded-[22px] border border-line-strong bg-surface p-6 md:min-h-72.5 md:p-8" data-scroll-reveal key={reason.number}>
              <span className="text-sm font-extrabold text-gold-bright">{reason.number}</span>
              ...
              <strong className="mt-auto border-t border-line pt-6 text-sm text-gold-bright">{reason.result}</strong>
            </article>
          ))}
        </div>
      </div>
