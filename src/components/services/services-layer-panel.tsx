  ...
  return (
    <aside className="min-h-125 rounded-[26px] border border-line-strong bg-surface/95 p-4.5 shadow-panel sm:p-6 lg:p-7.5" aria-label={ariaLabel}>
      <div className="grid h-full grid-cols-2 gap-3 sm:gap-6">
        {layers.map((layer) => (
          <article className="flex min-h-45 flex-col rounded-[18px] border border-line bg-surface px-4 py-5 sm:min-h-51 sm:px-5.5 sm:py-6" key={layer.title}>
            ...
            <p className="mt-2.5 text-sm text-muted">{layer.detail}</p>
            <span className="mt-auto block h-1.5 overflow-hidden rounded-full bg-line" aria-hidden="true">
