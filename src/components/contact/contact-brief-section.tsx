        ...
        <div>
          ...
          <ul className="mt-12 grid list-none grid-cols-1 gap-4 p-0 sm:grid-cols-2" aria-label={content.checklistAriaLabel}>
            {content.checklist.map((item) => (
              <li className="flex min-h-14.5 items-center gap-3 rounded-xl border border-line bg-surface px-4 py-3 text-sm font-semibold text-ink-soft" key={item}>
                <span className="grid size-5 shrink-0 place-items-center rounded-full bg-action text-sm font-black text-action-ink" aria-hidden="true">✓</span>{item}
              </li>
            ))}
          </ul>
        </div>
          ...
          <dl className="mt-6">
            {content.questions.map((item) => (
              <div className="border-t border-line py-4.5 first:border-t-0 first:pt-0 last:pb-0" key={item.question}>
                <dt className="text-sm font-bold text-ink-soft">{item.question}</dt>
                <dd className="mt-2 text-sm leading-5 text-muted">{item.answer}</dd>
              </div>
            ))}
          </dl>
        </aside>
