  ...
  return (
    <figure className="w-full min-w-0 overflow-hidden rounded-3xl border border-gold/35 bg-surface shadow-panel">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-5 py-4">
        <span className="text-sm font-bold tracking-wide text-gold-bright uppercase">{content.label}</span>
        <span className="inline-flex items-center gap-2 text-sm text-muted"><span className="size-1.5 rounded-full bg-success" aria-hidden="true" />{content.statusLabel}</span>
      </div>
      ...
      <figcaption className="p-5 sm:p-7">
        ...
        <div className="mt-6 grid grid-cols-1 gap-3 xs:grid-cols-2 [&_a]:px-4 [&_a]:text-sm">
